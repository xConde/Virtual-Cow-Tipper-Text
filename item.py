...
def build_item(item_type: str, less_likely: bool = False):
    ...
    item_weights = [35, 28, 24, 15, 12, 9, 6, 4, 2, 1] if not less_likely else [40, 30, 20, 10, 5, 3, 2, 1, 1, 1]
    ...
    item = Weapon(base_item['name'], min_stat, max_stat, rarity) if item_type == 'weapon' else Shield(base_item['name'], min_stat, max_stat, rarity)
...
def get_shop_items(cow_mood: str, player_cash: float, isLucky: bool):
    ...
    items = [
        {"label": "random item", "item": random_item_roll(less_likely), "price_multiplier": 1 * price_multiplier},
        {"label": "random weapon", "item": build_item('weapon', less_likely=False), "price_multiplier": 1.5 * price_multiplier},
        {"label": "random shield", "item": build_item('shield', less_likely=False), "price_multiplier": 2 * price_multiplier},
    ]
